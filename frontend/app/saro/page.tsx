...
// ── Subcategories for Saro (Women) ──
const SARO_FILTERS = [
  { label: "All", slug: "" },
  { label: "Bandhani", slug: "bandhani" },
  { label: "Banarsi", slug: "banarsi" },
  { label: "Silk", slug: "silk" },
  { label: "Dola Silk", slug: "dola-silk" },
  { label: "Georgette", slug: "georgette" },
  { label: "Chiffon", slug: "chiffon" },
  { label: "Leheriya", slug: "leheriya" },
  { label: "Chunariya", slug: "chunariya" },
  { label: "Kanjeevaram", slug: "kanjeevaram" },
  { label: "Net", slug: "net" },
  { label: "Paithani", slug: "paithani" },
  { label: "Kota Doria", slug: "kota-doria" },
  { label: "Patola", slug: "patola-saree" },
  { label: "Lucknavi", slug: "lucknavi" },
  { label: "Synthetic", slug: "synthetic-saree" },
  { label: "Blouse - Printed", slug: "blouse-printed" },
  { label: "Blouse - Plain", slug: "blouse-plain" },
];

const MAIN_CATEGORIES = [
  { label: "All", slug: "" },
  { label: "Saree", slug: "saree" },
  { label: "Poshak", slug: "poshak" },
  { label: "Lehengas", slug: "lehengas" },
  { label: "Rajputi Saree", slug: "rajputi-saree" },
];
...
export default function SaroShop() {
  ...
  const fetchProducts = useCallback(async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ gender: "women", sort: sortBy });
  ...
  return (
    <main style={{ background: "#0A0800", minHeight: "100vh", paddingTop: "100px" }}>
      <style>{`
        ...
        .saro-filter-btn:hover {
          border-color: rgba(201,168,76,0.5);
          color: #C9A84C;
        }
        .saro-filter-btn.active {
        ...
        @media (max-width: 768px) {
          .saro-grid { grid-template-columns: repeat(2, 1fr) !important; }
          .saro-sidebar { display: none !important; }
          .saro-mobile-filters { display: block !important; }
        }
        @media (max-width: 480px) {
          .saro-grid { grid-template-columns: 1fr !important; }
        }
      `}</style>
      ...
      >
        ...
        <h1 style={{
          ...
          animation: "saro-shimmer 12s linear infinite",
        }}>
          Saro
        </h1>
        <p style={{
          ...
          Grace woven into every thread — from timeless sarees to regal lehengas
        </p>
      ...
      <div style={{
        ...
        {MAIN_CATEGORIES.map((cat) => (
          <button
            key={cat.slug}
            className={`saro-filter-btn ${activeCategory === cat.slug ? "active" : ""}`}
      ...
      <div style={{
        ...
        <div className="saro-mobile-filters" style={{ display: "none" }}>
          <button
          ...
          {showFilters && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 20 }}>
              {SARO_FILTERS.map((f) => (
                <button
                  key={f.slug}
                  className={`saro-filter-btn ${activeFilter === f.slug ? "active" : ""}`}
                  onClick={() => setActiveFilter(f.slug === activeFilter ? "" : f.slug)}
                >
                  {f.label}
                </button>
                  ...
                  <div style={{ padding: "18px 20px 20px" }}>
                    ...
                    {product.variants && product.variants.length > 0 && (
                      <p style={{
                        ...
                        {Array.from(new Set(product.variants.map((v: Variant) => v.color))).length} colors available
                      </p>
                    )}

                    {/* Add to cart */}
                    <button
                      className="saro-add-btn"
